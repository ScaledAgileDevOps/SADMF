...
aliases: ["/roles/quality-authority/"]
---
...
The Quality Authority's test execution cycle is integrated into the [Convoy](/release-convoy/) timeline:

1. The [Source Management Team](/roles/source-management-team/) merges all feature branches into the Conflict branch and resolves all conflicts.
2. The Quality Authority receives the integrated build for testing.
3. The testing window is defined by the [Commodore](/roles/commodore/) and typically spans two to three weeks, during which the Quality Authority executes every test script in the regression suite plus all new scripts written for the current Convoy's features.
4. Defects discovered during testing are logged in the Defect Tracking Spreadsheet (a dedicated tab in the [Release Tracking](/practices/release-tracking/) spreadsheet) and assigned back to the [Code Engineers](/roles/code-engineer/) who wrote the offending code.
5. Code Engineers fix the defects, the [Source Management Team](/roles/source-management-team/) re-merges, and the Quality Authority re-executes the affected test scripts.
