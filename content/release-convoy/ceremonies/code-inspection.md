...
The inspection is conducted against the 247-item Code Compliance Checklist, a living document maintained jointly by the CSET and the [Enterprise Architecture Review Board (EARB)](/roles/enterprise-architecture-review-board/). The checklist covers critical concerns such as indentation consistency, variable naming conventions, comment density ratios (no fewer than one comment per four lines of code), maximum line length, brace placement style, and the proper ordering of import statements. Items related to algorithmic correctness, security vulnerabilities, or architectural soundness are not included on the checklist, as these concerns fall outside the CSET's area of expertise and are assumed to be handled elsewhere in the process. No one has verified this assumption.
